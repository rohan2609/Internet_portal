...
class Updater {
  ...
  async fetchVersion(channel, daysToStale = 30) {
    let v;
    try {
      if (!daysToStale || mtime(this.versionFile).isAfter((0, _moment2.default)().subtract(daysToStale, 'days'))) {
        v = await _fsExtra2.default.readJSON(this.versionFile);
      }
    } catch (err) {
  ...
  async update(manifest) {
    ...
    let unlock = await _rwlockfile2.default.write(this.updatelockfile, { skipOwnPid: true });
  ...
  async restartCLI() {
    await _rwlockfile2.default.read(this.updatelockfile);
    _rwlockfile2.default.unreadSync(this.updatelockfile);
  ...
  async autoupdate(force = false) {
    try {
      ...
      spawn(this.binPath, ['update'], { detached: true, stdio: ['ignore', fd, fd] }).on('error', e => this.out.warn(e, 'autoupdate:')).unref();
    } catch (e) {
  ...
  async checkIfUpdating() {
    if (await _rwlockfile2.default.hasWriter(this.updatelockfile)) {
      this.out.warn(`${this.config.name}: warning: update in process`);
      await this.restartCLI();
    } else await _rwlockfile2.default.read(this.updatelockfile);
  }
}
